...
其中，《汴京十二时辰》只是参考范式。后续项目可以是城市、人物、物件、档案、地方记忆、机构史、非遗、科技史或其他研究型展览主题。

## Why No Scripts In The First Release

第一版暂时不加入脚本或自动化工具。

这套 skills 的核心能力是策展判断、文本转译、结构拆解和视觉语言校准。这些工作具有较高自由度，需要根据不同研究主题、材料条件和展览对象做判断。脚本更适合处理稳定、重复、格式明确的任务，例如批量改文件、校验字段、生成目录或打包发布。

在这套方法仍然面向不同策展主题开放复用的阶段，过早加入自动化工具，容易把流程固定成某一种项目形态。第一版保持每个 skill 只有 `SKILL.md`，让结构更清楚、易读、易改，也更利于传播和二次使用。

后续版本可以考虑加入辅助脚本，但这些脚本应服务于工作流，而不是取代策展判断。例如：

- `validate_skills.py`：检查每个 skill 是否包含 `name`、`description`、中文触发词、参考范式和输出要求。
- `scaffold_project_docs.py`：为新项目生成“选题界定、研究转译、页面结构、视觉语言、日志复盘”等空白文档。
- `package_claude_skills.py`：把 Claude 版 skills 打包成可发布目录。

当某些动作在真实使用中被反复手工执行，再将它们抽成脚本会更稳。
